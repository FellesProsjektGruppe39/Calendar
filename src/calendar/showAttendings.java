			...
			@Override public void handle(ActionEvent e) {
				...
				for (int i = 0; i < cb.size(); i++) {
					...
					if (Integer.parseInt(info.getQuery()[i][7]) != a) {
						...
						if (a == 1) {
							Notif.create(Integer.parseInt(info.getQuery()[i][8]), CreateCalendar.getName(BID) + " har endret til attending mote paa id " + moteid.get(i));
						}
						if (a == 2) {
							Notif.create(Integer.parseInt(info.getQuery()[i][8]), CreateCalendar.getName(BID) + " har endret til not attending mote paa id " + moteid.get(i));
						}
